...
var (
	defaultLndDir     = btcutil.AppDataDir("lnd-bynd", false)
...
type chainConfig struct {
	...
	NodeT string `long:"node" description:"The blockchain interface to use." choice:"byndd" choice:"beyondcoind" choice:"neutrino"`
...
type config struct {
	...
	Bitcoin      *chainConfig    `group:"Beyondcoin" namespace:"beyondcoin"`
	BtcdMode     *btcdConfig     `group:"byndd" namespace:"byndd"`
	BitcoindMode *bitcoindConfig `group:"beyondcoind" namespace:"beyondcoind"`
...
func loadConfig() (*config, error) {
	defaultCfg := config{
		...
		Bitcoin: &chainConfig{
			Active:        true, // Force beyondcoin chain active
	...
	case cfg.Litecoin.Active:
		str := "%s: This implementation supports only Beyondcoin"
	...
	case cfg.Bitcoin.Active:
		...
		// know how to initialize the daemon.
		if numNets == 0 {
			str := "%s: either --beyondcoin.mainnet, or " +
				"beyondcoin.testnet, beyondcoin.simnet, or beyondcoin.regtest " +
				"must be specified"
			err := fmt.Errorf(str, funcName)
			return nil, err
		}
		...
		case "byndd":
			...
			if err != nil {
				err := fmt.Errorf("unable to load RPC "+
					"credentials for byndd: %v", err)
				return nil, err
			}
		case "beyondcoind":
			if cfg.Bitcoin.SimNet {
				return nil, fmt.Errorf("%s: beyondcoind does not "+
					"support simnet", funcName)
			}
			...
			if err != nil {
				err := fmt.Errorf("unable to load RPC "+
					"credentials for beyondcoind: %v", err)
				return nil, err
			}
		case "neutrino":
		...
		default:
			str := "%s: only byndd, beyondcoind, and neutrino mode " +
				"supported for beyondcoin at this time"
			return nil, fmt.Errorf(str, funcName)
		}
...
func parseRPCParams(cConfig *chainConfig, nodeConfig interface{}, net chainCode,
	...
	if cConfig.SimNet {
		str := "%v: rpcuser and rpcpass must be set to your byndd " +
